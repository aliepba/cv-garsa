...
class SearchManageController extends Controller
{
    ...
    public function searchPage($word)
    {
        ...
        foreach ($kd_transaction as $kode) {
            ...
            ->first();
            $dashboard_content .= 'Riwayat Transaksi Terbaru : (Kode Transaksi : ' . $kode->kode_transaksi . ', Total : ' . number_format($ket_transaksi->total,2,',','.') . ', sales : ' . $ket_transaksi->sales . ', Waktu : ' . Carbon::parse($ket_transaksi->created_at)->diffForHumans() . ') ';
        }
    	// Profile Content
    	...
    	// Transaksi
    	$transaksi_content = 'Transaksi => Daftar Pesanan, Tanggal Transaksi, Waktu, sales, Subtotal, Diskon, Total, Nominal Bayar dan Kembali';
    	// Laporan Transaksi
    	...
    	foreach ($accounts as $account) {
    		$transaction_activity = Transaction::where('id_sales', $account->id)->select('kode_transaksi')->distinct()->get();
